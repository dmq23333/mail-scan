...
public class JobApplicationService {
    ...
    private static final int MAX_SYNC_RESULTS = 50;
    ...
    public List<JobApplicationDTO> findByStatus(OfferStatus status) {
        if (status == null || status == OfferStatus.ALL) return findAll();
    ...
    public List<JobApplicationDTO> syncFromGmail(OAuth2AuthorizedClient client, String customQuery)
        ...
        ListMessagesResponse listResponse = gmail.users().messages().list("me")
            .setQ(query)
            .setMaxResults((long) MAX_SYNC_RESULTS)
            .execute();

        List<JobApplicationDTO> synced = new ArrayList<>();
        if (listResponse.getMessages() == null) return synced;

        for (Message stub : listResponse.getMessages()) {
            // Skip messages already in DB
            if (repository.existsByGmailMessageId(stub.getId())) continue;
    ...
    public JobApplicationDTO update(Long id, JobApplicationDTO.UpdateRequest req) {
        ...
        if (req.getJobTitle()    != null) app.setJobTitle(req.getJobTitle());
        if (req.getPlatform()    != null) app.setPlatform(req.getPlatform());
        ...
        if (req.getMark()        != null) app.setMark(req.getMark());
        if (req.getJd()          != null) app.setJd(req.getJd());
