...
public class MailParser {
    ...
    private static final List<Pattern> JOB_TITLE_PATTERNS = List.of(
        ...
        Pattern.compile("(?i)interview\\s+(?:invitation|invite)\\s*[:\\-]\\s*(.+?)\\s+(?:at|to|with|–|-)", Pattern.CASE_INSENSITIVE),
        Pattern.compile("(?i)(?:re|regarding):\\s*(.+?)\\s+(?:at|to|–|-|\\[)", Pattern.CASE_INSENSITIVE),
        Pattern.compile("(?i)(.+?)\\s*[-–]\\s*application\\s+(?:received|confirmed|submitted)", Pattern.CASE_INSENSITIVE),
    ...
    private static final List<Pattern> COMPANY_FROM_SUBJECT_PATTERNS = List.of(
        Pattern.compile("(?i)(?:at|to|from|with)\\s+([A-Z][\\w\\s&,.']+?)(?:\\s*[,!\\?]|\\s*$|\\s+(?:has|is|for|have|about|team|\\())", Pattern.CASE_INSENSITIVE),
        Pattern.compile("(?i)application\\s+to\\s+([A-Z][\\w\\s&,.']+?)(?:\\s*[:\\-,!]|\\s*$)", Pattern.CASE_INSENSITIVE),
